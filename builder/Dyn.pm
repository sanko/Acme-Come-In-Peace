...
sub process_xs {
    ...
    my $ob_file = $builder->compile(
        ...
        include_dirs => [ curdir, dirname($source) ]
    );
    require DynaLoader;
    my $mod2fname
    ...
    my $lib_file
        = catfile( $archdir, $mod2fname->( \@parts ) . '.' . $options->{config}->get('dlext') );
    return $builder->link(
        objects     => [ $ob_file, process_asm($options) ],
        lib_file    => $lib_file,
        module_name => join '::',
        @parts
    );
...
sub process_asm {
    my ($options) = @_;
    my @obj;
    for my $source ( find( qr/\.S$/i, 'lib' ) ) {
        warn $source;
        my ( undef, @parts ) = splitdir( dirname($source) );
        push @parts, my $file_base = basename( $source, '.S' );
        my $tempdir = 'temp';
        require ExtUtils::CBuilder;
        my $builder = ExtUtils::CBuilder->new( config => $options->{config}->values_set );
        push @obj,
            $builder->compile(
            extra_compiler_flags => [qw[-fno-pie -no-pie]],
            source               => $source,
            defines              => {},
            include_dirs         => [ curdir, dirname($source) ]
            );
